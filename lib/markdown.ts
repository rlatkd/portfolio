...
import { getPostContents } from '@/utils/postContents';
...
export async function getPosts() {
  noStore();
  return getMDXData(path.join(process.cwd(), 'content', 'posts')) // 정적파일경로
}

export async function getEtcMdxs() {
  return getMDXData(path.join(process.cwd(), 'content', 'etc'))
}
