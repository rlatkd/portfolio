...
summary: '문득 개발을 하다가 @~ArgsConstructor에 대해 궁금증이 생겼다.자연스럽게 필요한 코드에 해당 어노테이션들을 달지만 왜 달아야하는지 다시 공부한다.간단하게 말하면 해당 어노테이션들은 다음과 같다.@N...'
---
...
_@NoArgsConstructor_을 사용하면, class에 명시적으로 선언된 생성자가 없더라도 instance를 생성할 수 있다.
```java
@NoArgsConstructor
public class User {
...
_@AllArgsConstructor_을 사용하면, class의 모든 field를 한 번에 초기화할 수 있다.
```java
@AllArgsConstructor
public class User {
...
_@RequiredArgsConstructor_을 사용하면, class가 의존하는 field를 간단하게 초기화할 수 있다.
