...
export const strengths: Strength[] = [
  {
    ...
    points: [
      '정산내역과 정산대상 결제내역의 건수·금액 불일치 원인을 원장 테이블 직접 대조로 규명. 카드 매입 전 취소, CMS 배치 미처리, 휴대전화 정산테이블의 결제일 컬럼 오염 등 결제수단별로 원인과 성격이 상이함을 확인',
      '판정 규칙을 소비자에 복제하지 않고, 기존 소비자에 영향이 없는 선택적 파라미터를 스펙으로 설계해 계정계 API 확장을 요청',
      '계정계 원장 데이터와 직접 대조해 결제일·정산일 기준과 결제수단 조합 전반에서 정합성 확인',
    ],
    result: [
  ...
  },
  {
    ...
    points: [
      '수 년째 방치되던 계정계 다운 장애의 원인 규명. 내려간 것은 DB가 아닌 애플리케이션 서버였고, 한계 지점은 쿼리가 아니라 한 응답에 담기는 행 수',
      '1,000건 단위 페이징 순회 호출과 SXSSF 스트리밍으로 계정계와 배치 양쪽의 메모리 상주 행 수를 데이터 총량과 무관한 상수로 고정',
      '병렬 호출은 적용하지 않음. 공용 시스템의 안정성을 추출 시간보다 우선',
    ],
    result: [
  ...
  },
  {
    ...
    points: [
      '결과를 받기 전에 결제중으로 바꾸는 기존 구조가 이중 출금을 막는 장치임을 확인하고, 전송 로직은 건드리지 않은 채 예외 경로에만 개입',
      '되돌려도 안전한 실패(명시적 거부)와 되돌리면 안 되는 실패(타임아웃)를 구분하는 정정 규칙 수립. 요청이 실제로 나갔는지 확정 가능한 건만 복구',
    ],
    result: [
  ...
  },
  {
    ...
    points: [
      '외부 시스템의 예외 메시지가 비고 컬럼을 통해 고객사 화면에 그대로 노출되던 문제를 AOP 기반 경계 변환으로 차단',
      '기록(로그)·표시(안내 문구)·전달(운영자 알림)의 대상을 분리. 실패를 고객 문의가 아니라 시스템 알림으로 인지',
    ],
    result: [
  ...
  },
  {
    ...
    points: [
      '입사 직후 투입된 첫 프로젝트. 종이 계약서 기반 자동이체 계약을 비대면 전자계약으로 전환하는 신규 서비스를 기획부터 운영까지 구축',
      '파트너 공식 API 스펙만으로는 흐름을 구성할 수 없어 협의를 통해 스펙 확장. CMS+가 발급한 식별자를 콜백으로 되돌려받아 계약을 특정',
      '링크 서명 확장으로 "납부자 등록 후 계약 체결"을 "계약 체결 후 납부자 등록"으로 흐름 역전',
    ],
    result: [
